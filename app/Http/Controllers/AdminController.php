...
class AdminController extends Controller
{
    ...
    public function editFarmlistPost(Request $req){
    	$editing = FarmList::where('id', $req->id)->first();
        ...
        if(!empty($posts)) {
            ...
            foreach ($posts as $post) {
                ...
                if ($post->type == 'deposits' && $post->status == 'pending') {
                    ...
                    $links .= '<a href="/admin/transactions/deposits/decline/' . $post->id . '" class="dropdown-item" onclick="return confirm("Are you sure you want to decline this payouts?");">Decline</a>';
    ...
    public function approveDeposits($id){
        ...
        $title= ' ';
        $name = $used->name;
        $content = 'Your deposit of N'.number_format($transaction->amount,2).' into '.$depositLocation.' has been approved.<br><br> You currently have the total balance of N'.number_format($balance,2).' in your wallet.';
        $button = false;
        $button_text = '';
    ...
    public function declineDeposits($id){
        ...
        $title= ' ';
        $name = $used->name;
        $content = 'Your deposit of N'.number_format($trans->amount,2).' has been declined.<br><br>';
        $button = false;
        $button_text = '';
    ...
    public function approvePayouts($id){
        ...
        $title= ' ';
        $name = $used->name;
        $content = 'Your payout request of N'.number_format($transaction->amount,2).' has been approved.<br><br> You currently have the total balance of N'.number_format($new,2).' in your wallet.';
        $button = false;
        $button_text = '';
...
//                                        </a>';
    ...
    public function loadPayouts(Request $request)
    {
        ...
        if(empty($request->input('search.value')))
        {
            $posts = Transaction::where('type','payouts')->offset($start)
                ->limit($limit)
                ->orderBy($order,$dir)
                ->get();
        } else {
    ...
    public function payOutWalletPost(Request $req){
        ...
        if($req->amount > $wallet->total_amount){
            ...
            $title= ' ';
            $name = $member->name;
            $content = 'Your just got a payout of N'.number_format($req->amount,2).'.<br><br> You currently have the total balance of N'.number_format($new,2).' in your wallet.';
            $button = false;
            $button_text = '';
...
//        $deposits = Transaction::where(['type'=>'deposits'])->latest('id')->get();
    ...
    public function withdrawForUser(Request $request)
    {
        ...
        $title= ' ';
        $name = $user->name;
        $content = 'Your have requested for a payout of N'.number_format($request->amount,2).' from your current '. $type .' balance of N'.number_format($balance,2).'. ';
        $button = false;
        $button_text = '';
    ...
    public function depositForUser(Request $request)
    {
        ...
        $title= ' ';
        $name = $user->name;
        $content = 'Your deposit of N'.number_format($request->amount,2).' has been booked successfully.<br>Pending system approval. <br> We will update the status of your transaction within 24hours.';
        $button = false;
        $button_text = '';
    ...
    public function payMilestone($id){
        ...
        $amount = 0;
        if(count($milestones) == count($paid)){
            $amount += implode("", explode(',',$investment->amount_invested))+implode("", explode(',',$investment->milestoneReturns()));
        }else {
            $amount += implode("", explode(',',$investment->milestoneReturns()));
        }
        $data = [
            'amount'=>$amount,
