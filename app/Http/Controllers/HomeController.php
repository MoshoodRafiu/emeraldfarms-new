...
class HomeController extends Controller
{
    ...
    public function pending(){
        ...
        if(Util::completeProfile($user))
            ...
            return view('user.farmlist', ['farmlists'=>$farmlists, 'opened'=>$opened, 'closed'=>$closed, 'pending'=>$pending, 'pend'=>true, 'type' => $type]);
    }
    ...
    public function closed(){
        ...
        if(Util::completeProfile($user))
            ...
            return view('user.farmlist', ['farmlists'=>$farmlists, 'opened'=>$opened, 'closed'=>$closed, 'pending'=>$pending, 'close'=>true, 'type' => $type]);
    }
    ...
    public function opened(){
        ...
        return view('user.farmlist', ['farmlists'=>$farmlists, 'opened'=>$opened, 'closed'=>$closed, 'pending'=>$pending, 'open'=>true, 'type' => $type]);
    }
        ...
        if(Util::compareTranaction($trx)){
            ...
            if($confirm->data->status == 'success'){
                if($type=='investments'){
                    ...
                    $title= '';
                    $name = Admin::first()->name;
                    $content = 'A deposit request of N'.number_format($amount,2). ' in '. $depositLocation.  ' has been done successfully via paystack funding by '.$user->name;
                    $button = true;
                    ...
                    $title= ' ';
                    $name = $user->name;
                    $content = 'Your farm booking has been successful. Funds in emerald bank will be automatically used to sponsor a farm for you.';
                    $button = false;
                    $button_text = '';
                    ...
                    $title= ' ';
                    $name = Admin::first()->name;
                    $content = 'A farm booking has been successfully created by ' . $user->name . 'in ' . $farmlist->title . 'farm';
                    $button = true;
        ...
        if($banks < 1){
            ...
            if(count($transa) > 0){
                ...
                if($req->amount > $wallet->total_amount){
                    ...
                    $title= ' ';
                    $name = $user->name;
                    $content = 'Your have requested for a payout of N'.number_format($req->amount,2).' from your current wallet balance of N'.number_format($wallet->total_amount,2).'. ';
                    $button = false;
                    $button_text = '';
                    ...
                    $title= '';
                    $name = Admin::first()->name;
                    $content = 'A payout request of N'.number_format($req->amount,2).' has been initiated by '. $user->name.' from the current wallet balance of N'.number_format($wallet->total_amount,2).'. Kindly login to approve or decline';
                    $button = true;
    ...
    public function addDepositPost(Request $req)
    {
        ...
        if($req->method == 'bank'){
            ...
            $title= '';
            $name = Admin::first()->name;
            $content = 'A deposit request of N'.number_format($req->amount,2).' has been booked by '.$user->name.'<br>Pending your approval. Kindly login to your dashboard to approve';
            $button = true;
            $button_text = 'View Deposit';
    ...
    public function profile(){
        ...
        if($user->welcome_mail != 'sent'){
            ...
            $title= ' ';
            $name = '';
            $content = 'Welcome to Emerald Farms '.ucwords($user->name).'.<br>You are about to start a journey of fulfilment.<br><br>We are excited to have you on board as together we can develop the Agricultural Sector in Nigeria, ensure food security, environmental sustainability and economic opportunities.<br><br>If you have any questions or enquiries, please do not hesitate to write or call any of our hotlines.';
            $button = false;
            $button_text = '';
            $button_link = '';
    ...
    public function editBankPost($id , Request $req){
        $user = Util::getUser();
        $bank = Bank::where('user', $user->email)->whereId($id)->first();
        if(! $bank){
