...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    protected $fillable = [
        ...
        'state', 'country', 'zip', 'bank_name', 'city',
        'account_number', 'account_name', 'passport',
        'welcome_mail','dob','nk_Name','nk_Phone','nk_Email',
        'nk_Address','code'
    ];
    ...
    {
        do{
            $code = Str::slug($this->name) . '-' . Str::random(3);
        }while(static::whereCode($code)->first() != NULL);


        return $this->update(['code' => $code]);
    }
    ...
    public function emeraldbank()
    {
        return $this->hasOne(Farmfund::class);
    }
    ...
    {
        return $this->hasMany(Verified::class);
    }

    public function totalInvestmentsAmount()
    {
        return Investment::where(['user'=>$this->email,'status'=>'active'])->sum('amount_invested');
    }

    public function hasCompletedProfile()
    {
        return $this->address != '' && $this->state != '' && $this->country != '' && $this->zip != '' && $this->city != '' && $this->dob != '';
    }

    public function hasCompletedKinProfile()
    {
        return $this->nk_Name != NULL && $this->nk_Address != NULL && $this->nk_Phone != NULL && $this->nk_Email != NULL;
    }
}
